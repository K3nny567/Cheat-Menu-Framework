...
module CheatUtils
  def self.heal_player
    player_heal(999, true)
    player_feed(999, true)
    player_rest(999, true)
  end
  ...
  def self.player_heal(value, mode)
    ...
    when 'OnlyBelow'
      $game_player.actor.health += @health_rate if $game_player.actor.health < @health_limit
    when 'DropsBelow'
      $cheat_infinite_health_loop = true if $game_player.actor.health < @health_limit
      if $game_player.actor.health < @health_max && $cheat_infinite_health_loop
        $game_player.actor.health += @health_rate
      end
      $cheat_infinite_health_loop = false if $game_player.actor.health >= @health_max && $cheat_infinite_health_loop
    end
  end
  ...
  def self.player_feed(value, mode)
    ...
    when 'OnlyBelow'
      $game_player.actor.sat += @food_rate if $game_player.actor.sat < @food_limit
    when 'DropsBelow'
      $cheat_infinite_food_loop = true if $game_player.actor.sat < @food_limit
      $game_player.actor.sat += @food_rate if $game_player.actor.sat < @food_max && $cheat_infinite_food_loop
      $cheat_infinite_food_loop = false if $game_player.actor.sat >= @food_max && $cheat_infinite_food_loop
    end
  end
  ...
  def self.player_rest(value, mode)
    ...
    when 'OnlyBelow'
      $game_player.actor.sta += @stamina_rate if $game_player.actor.sta < @stamina_limit
    when 'DropsBelow'
      $cheat_infinite_stamina_loop = true if $game_player.actor.sta < @stamina_limit
      $game_player.actor.sta += @stamina_rate if $game_player.actor.sta < @stamina_max && $cheat_infinite_stamina_loop
      $cheat_infinite_stamina_loop = false if $game_player.actor.sta >= @stamina_max && $cheat_infinite_stamina_loop
    end
  end
  ...
  def self.load_infinite_stats_config
    $cheat_infinite_health_limit = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config',
                                                         'HealthRegenLimit', 0.20)
    $cheat_infinite_health_rate = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config', 'HealthRegenRate',
                                                        0.01)
    $cheat_infinite_food_limit = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config', 'FoodRegenLimit',
                                                       0.10)
    $cheat_infinite_food_rate = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config', 'FoodRegenRate',
                                                      0.005)
    $cheat_infinite_stamina_limit = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config',
                                                          'StaminaRegenLimit', 0.40)
    $cheat_infinite_stamina_rate = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config',
                                                         'StaminaRegenRate', 0.002)
    $cheat_infinite_health_mode = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config', 'HealthRegenMode',
                                                        'OnlyBelow')
    $cheat_infinite_food_mode = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config', 'FoodRegenMode',
                                                      'OnlyBelow')
    $cheat_infinite_stamina_mode = FileGetter.cheat_load('Cheats Mod - Infinite Main Stats: Config',
                                                         'StaminaRegenMode', 'OnlyBelow')
  end
  ...
  def self.save_infinite_stats_config
    ...
    FileGetter.cheat_save('Cheats Mod - Infinite Main Stats: Config', 'StaminaRegenLimit',
                          $cheat_infinite_stamina_limit)
...
module CheatsMod
  ...
  def self.cheat_triggers
    ...
    CheatUtils.player_heal(nil, false) if $cheat_infinite_health
    CheatUtils.player_feed(nil, false) if $cheat_infinite_food
